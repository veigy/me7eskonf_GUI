...
class EskonfTool(ctk.CTk):
    def __init__(self):
        super().__init__()
        self.title("ME7.5 ESKONF Editor v1.1")
        ...
        self.optionmenu = ctk.CTkOptionMenu(self.sidebar, values=self.layouts, command=self.optionmenu_callback, variable=self.optionmenu_var)
        self.optionmenu.pack(padx=20, pady=10)
        self.optionmenuNum = 0

        self.ext_search_var = ctk.BooleanVar(value=False)
        self.check_ext = ctk.CTkCheckBox(self.sidebar, text="Extended Search (FF FF 00)", variable=self.ext_search_var, font=("Arial", 11))
        ...
        self.btn_save = ctk.CTkButton(self.sidebar, text="Save Modified BIN", fg_color="#c0392b", hover_color="#a93226", command=self.save_bin, state="disabled")
        ...
        ctk.CTkLabel(self.about_frame, text="Version 1.1 | 2026", font=("Arial", 10), text_color="gray").pack()
    ...
    def create_table_slot(self, idx):
        ...
        p_btns = [("KAT", ["LSHHK"]), ("N249", ["ULT"]), ("SAI", ["SLV", "SLP"]), ("EVAP", ["TEV"]), ("VVT", ["NWS"])]
        for i, (text, comps) in enumerate(p_btns):
            btn = ctk.CTkButton(preset_frame, text=text, width=75, height=24, font=("Arial", 10), fg_color="#34495e", command=lambda c=comps, s=idx: self.apply_preset(s, c))
        ...
        for b_idx in range(7):
            ...
            for p_idx in range(4):
                combo = ctk.CTkOptionMenu(row, values=["Y (00)", "? (01)", "S (10)", "N (11)"], width=130, height=30, text_color="black", font=("Arial", 13, "bold"))
                combo.grid(row=0, column=p_idx+1, padx=4, pady=2); combos.append(combo)
                comboLabel = ctk.CTkLabel(row, text="", font=("Arial", 10), text_color="white", wraplength=120)
                comboLabel.grid(row=1, column=p_idx+1, pady=(0,2))
                comboLabels.append(comboLabel)

        return {"frame": addr_frame, "title": title_lbl, "orig_hex": orig_hex_lbl, "mod_hex": mod_hex_lbl, "btn_copy": btn_copy, "combos": combos, "byte_labels": byte_labels, "comboLabels": comboLabels}

    def parse_eskonf_strict(self, data):
        res = []
        patterns = [b'\xAA\xFF\x00']
        if self.ext_search_var.get(): patterns.append(b'\xFF\xFF\x00')
        for pat in patterns:
            start = 0
            while True:
                ...
                if len(b) == 7 and all(x != 0 for x in b[3:7]):
                    p = []
                    for byte in b: p.extend([(byte >> 6) & 0x03, (byte >> 4) & 0x03, (byte >> 2) & 0x03, byte & 0x03])
    ...
    def apply_preset(self, slot_idx, components):
        ...
        current_map = self.comp_map[self.optionmenuNum]
        for comp in components:
            idx = current_map.get(comp)
    ...
    def refresh_hex(self, slot_idx):
        ...
        for b_idx in range(7):
            ...
            current_bits.extend(bits)
            val = (bits[0] << 6) | (bits[1] << 4) | (bits[2] << 2) | bits[3]
            new_hex.append(val); slot["byte_labels"][b_idx].configure(text=f"BYTE {b_idx}: {val:02X}")

        # Zvýraznění změn
        orig_item = next((item for item in self.found_results if item['addr'] == slot.get("current_addr")), None)
        if not orig_item and hasattr(slot, "manual_orig_bits"): orig_item = {"bits": slot.manual_orig_bits}

        if orig_item:
            for i, (curr, orig) in enumerate(zip(current_bits, orig_item["bits"])):
                if curr != orig:
                    slot["comboLabels"][i].configure(text_color="#f1c40f", font=("Arial", 10, "bold"))
                else:
                    slot["comboLabels"][i].configure(text_color="white", font=("Arial", 10))
    ...
    def fill_table(self, slot_idx, item):
        slot = self.tables[slot_idx]
        slot["current_addr"] = item.get("addr")
        if item.get("is_manual"): slot.manual_orig_bits = item["bits"]
        ...
        self.optionmenu_callback(self.optionmenu_var.get()) # Refresh labels
        self.refresh_hex(slot_idx)
    ...
    def save_bin(self):
        ...
        if file_path:
            ...
            self.checksum_warning.configure(text="SAVED! FIX CHECKSUM!")
            messagebox.showwarning("Checksum Warning", "File saved! FIX CHECKSUM before flashing!")
    ...
    def optionmenu_callback(self, choice):
        ...
        for table in self.tables:
            for i, comboLabel in enumerate(table["comboLabels"]):
                comboLabel.configure(text=self.comp_names[self.optionmenuNum][i])
